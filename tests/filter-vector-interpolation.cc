...
BOOST_AUTO_TEST_CASE_TEMPLATE (filter_vector_interpolation_test, T, functionTypes_t)
{
  ...
  typename VectorInterpolation<T>::vector_t params
    = typename VectorInterpolation<T>::vector_t (cosinusVector);
  typename VectorInterpolation<T>::size_type outputSize = 1;

  boost::shared_ptr<VectorInterpolation<T> >
    interpolation = vectorInterpolation<T> (params, outputSize);

  BOOST_CHECK (interpolation->outputSize () == 1);

  typename GenericDifferentiableFunction<T>::argument_t x2 (1);
  x2.setZero ();
  for (unsigned i = 0; i < 30; ++i)
    {
      x2.coeffRef (0) = i;
      std::cout
	<< (*interpolation) (x2) << "\n"
	<< interpolation->gradient (x2, 0) << "\n"
	<< interpolation->jacobian (x2) << std::endl;
    }
}
