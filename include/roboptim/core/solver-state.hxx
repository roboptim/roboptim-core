...
namespace roboptim
{
  ...
  template <typename P>
  const typename SolverState<P>::argument_t&
  ...
  template <typename P>
  typename SolverState<P>::argument_t&
  SolverState<P>::x ()
  {
    return x_;
  }
...
# ifdef ROBOPTIM_PRECOMPILED_DENSE_SPARSE
  extern template class ROBOPTIM_CORE_DLLAPI SolverState<Problem<EigenMatrixDense> >;
  extern template class ROBOPTIM_CORE_DLLAPI SolverState<Problem<EigenMatrixSparse> >;
