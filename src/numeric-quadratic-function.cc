...
namespace roboptim
{
  ...
  // 1/2 * x^T * A * x + b^T * x
  void
  NumericQuadraticFunction::impl_compute
  ...
  // x * A + b
  void
  NumericQuadraticFunction::impl_gradient
  (result_t result, argument_t x, size_type) const throw ()
  {
    result.block (0, 0, result.rows (), result.cols ()).noalias () = a_ * x;
    result += b_;
  }

  // A
  void
  NumericQuadraticFunction::impl_hessian
  (hessian_t hessian, argument_t, size_type) const throw ()
  {
    hessian.block (0, 0, hessian.rows (), hessian.cols ()).noalias () = a_;
  }
